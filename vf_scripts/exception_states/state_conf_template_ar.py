...
APPEALS_COURT_IMPORT['udcs'].update({'source':'ARVF','type':'special_1_judicial_district'})
...
DISTRICT_COURT_IMPORT['udcs'].update({'source':'ARVF','type':'special_2_judicial_district'})
...
JUSTICE_PEACE_IMPORT = dict(tsd.td.DEFAULT_VF_TABLE)
JUSTICE_PEACE_IMPORT['udcs'] = dict(tsd.td.DEFAULT_VF_TABLE['udcs'])
JUSTICE_PEACE_IMPORT['udcs'].update({'source':'ARVF','type':'special_3_county_council'})
JUSTICE_PEACE_IMPORT.update({
    'table':'electoral_district_special_3_import',
    'columns':{
        'name':tsd.VFMAX+3,
        'identifier':{'function':tsd.td.reformat.ed_concat,'columns':(tsd.VFMAX+3,),'defaults':{'type':'special_3_county_council'}},
        'id_long':{'function':tsd.td.reformat.ed_concat,'columns':(tsd.VFMAX+3,),'defaults':{'type':'special_3_county_council'}}
        },
    })

JUSTICE_PEACE_ACTUAL = dict(tsd.td.DEFAULT_ACTUAL_TABLE)
JUSTICE_PEACE_ACTUAL.update({
    'schema_table':'electoral_district',
    'import_table':JUSTICE_PEACE_IMPORT,
    'long_fields':({'long':'id_long','real':'id'},),
    'long_from':('id_long',),
    'distinct_on':('id_long',),
    })

SCHOOL_NAME_IMPORT = dict(tsd.td.DEFAULT_VF_TABLE)
SCHOOL_NAME_IMPORT['udcs'] = dict(tsd.td.DEFAULT_VF_TABLE['udcs'])
SCHOOL_NAME_IMPORT['udcs'].update({'source':'ARVF','type':'special_4_school_district'})
SCHOOL_NAME_IMPORT.update({
    'table':'electoral_district_special_4_import',
    'columns':{
        'name':tsd.VFMAX+4,
        'identifier':{'function':tsd.td.reformat.ed_concat,'columns':(tsd.VFMAX+4,),'defaults':{'type':'special_4_school_district'}},
        'id_long':{'function':tsd.td.reformat.ed_concat,'columns':(tsd.VFMAX+4,),'defaults':{'type':'special_4_school_district'}}
        },
    })

SCHOOL_NAME_ACTUAL = dict(tsd.td.DEFAULT_ACTUAL_TABLE)
SCHOOL_NAME_ACTUAL.update({
    'schema_table':'electoral_district',
    'import_table':SCHOOL_NAME_IMPORT,
    'long_fields':({'long':'id_long','real':'id'},),
    'long_from':('id_long',),
    'distinct_on':('id_long',),
    })
...
EXTRA_DISTRICTS = OrderedDict({
        'special_1_judicial_district':{'filename':'20121018_AR_CourtofAppeals.txt','column':2,'prepend':()},
        'special_2_judicial_district':{'filename':'20121018_AR_CourtofAppeals.txt','column':3,'prepend':()},
        'special_3_county_council':{'filename':'20121018_AR_CourtofAppeals.txt','column':4,'prepend':('county_id',)},
        'special_4_school_district':{'filename':'20121019_AR_SchoolNames.txt','column':2,'prepend':()},
        })
ACTUAL_TABLES = (
        APPEALS_COURT_ACTUAL,
        DISTRICT_COURT_ACTUAL,
        JUSTICE_PEACE_ACTUAL,
        SCHOOL_NAME_ACTUAL,
        tsd.PRECINCT_ACTUAL,
        tsd.LOCALITY_ACTUAL,
...
ERSATZPG_CONFIG.update({
    ...
    'tables':{
        ...
        'special_3_county_council':JUSTICE_PEACE_IMPORT,
        'special_4_school_district':SCHOOL_NAME_IMPORT,
