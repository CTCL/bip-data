...
ed_map.update(dict([('{state} Judicial Superior {number}'.format(state=state[0], number=(numberclean(jdpat.match(n).groupdict()['number']+jdpat.match(n).groupdict()['extra']) if jdpat.match(n) else n)).lower(),{'name':n,'type':'special_1_judicial_district'}) for n in special_judicial_district]))
