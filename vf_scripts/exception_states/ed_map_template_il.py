...
for county in county_id:
    ...
    ed_map.update({'{name} County'.format(name=county).lower():{'name':county.replace('jo daviess','jodaviess'),'type':'county'}})
county_id.append('JO DAVIESS')
...
for county in county_council:
    ...
    old_county_name = county_name.replace('jo daviess','jodaviess')
    district_stuff = county.replace(county_name,'').strip()
    m = re.match(r'[A-Za-z# ]*(?P<district_number>[0-9]+)[A-Za-z\(\) ]*',county)
    if not m:
        p = re.compile(r'[A-Za-z# ]*\s(?P<district_number>[IViv]+)')
        m = p.match(county)
