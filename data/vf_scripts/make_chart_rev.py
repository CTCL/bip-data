...
def make_char(dist_type):
    with open('precinct_report.csv','r') as f, open(os.path.join(ss.script_home,'charts',dist_type+'.html'),'w') as g:
        ...
        for line in csvr:
            data.append({'idx':csvr.line_num-2,'count':float(line[dist_type])/float(line['total']),'min':1 - float(line['min {t}'.format(t=dist_type)]),'first':1- float(line['1st {t}'.format(t=dist_type)]),'third':1- float(line['3rd {t}'.format(t=dist_type)]),'max':1-float(line['max {t}'.format(t=dist_type)]),'state':line['state_name']})
